...
const supportedHostnames = new Set(["chatgpt.com", "chat.openai.com"]);
...
export class ChatGPTAdapter implements ProviderAdapter {
  ...
  matchesLocation(location: Location): boolean {
    return supportedHostnames.has(location.hostname);
  }
  ...
  findComposer(): HTMLElement | null {
    ...
    if (sendForm !== undefined && sendForm !== null) {
      const relatedComposer = findFirstElement(sendForm, composerSelectors);
      if (relatedComposer !== null) {
        return relatedComposer;
      }
    }

    return findFirstElement(this.document, composerSelectors);
  }
  ...
  setComposerText(text: string): void {
    ...
    // ProseMirror updates its internal state only through a native editing transaction.
  ...
  findSendButton(): HTMLElement | null {
    return findFirstElement(this.document, sendButtonSelectors);
  }
  ...
  isSendAction(event: Event): boolean {
    ...
    return sendButtonSelectors.some(
